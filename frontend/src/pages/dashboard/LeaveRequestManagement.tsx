// src/pages/LeaveRequestDashboard.jsx
import React, { useState, useEffect, useRef } from 'react';
import {
  Plus, Edit, Trash2, Search, ChevronDown, Eye, ChevronLeft, ChevronRight,
  AlertTriangle, CheckCircle, XCircle, X, AlertCircle, RefreshCw, Filter,
  Grid3X3, List, Menu, Upload, Image as ImageIcon, Calendar, Clock, User,
  Check, AlertOctagon
} from 'lucide-react';
...
const LeaveRequestDashboard = () => {
  ...
  const { role } = useOutletContext<{ role: string }>(); // 'employee' or 'company'
  ...
  const handleFilterAndSort = () => {
    ...
    if (searchTerm.trim()) {
      filtered = filtered.filter(
        (l) =>
          formatLeaveType(l.type)?.toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  const handleCreateOrUpdateLeave = async () => {
    ...
    formData.attachments.forEach((file) => fd.append('attachments[]', file)); // Fixed
  ...
  const handleRejectLeave = async () => {
    ...
    if (!canApproveReject(rejectConfirm)) return;
    try {
      setOperationLoading(true);
              ...
              >
                <td className="py-3 px-4">
                  <div className="flex items-center space-x-2">
                    ...
                    <span className="font-medium text-gray-900">{leave.employee?.name || '—'}</span>
                  </div>
                </td>
                ...
                <td className="py-3 px-4 text-right float-end">{renderActions(leave)}</td>
              </motion.tr>
            ))}
          </tbody>
        </table>
  ...
  /* ────────────────────────────────────────────────────────────── */
  /* ── GRID & LIST VIEWS (Use renderActions) ──────────────────── */
  /* ────────────────────────────────────────────────────────────── */
