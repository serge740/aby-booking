...
const [approveReason, setApproveReason] = useState<string>('');
  ...
  (newLeave: LeaveRequest) => {
    ...
    if (isEmployee && newLeave.employeeId === employeeId){
...
const handleApproveLeave = async () => {
  ...
  try {
    setOperationLoading(true);
    // Pass the reason (can be empty/optional)
    await leaveService.approveLeave(approveConfirm.id, approveReason.trim());
    setApproveConfirm(null);
    setApproveReason(''); // reset
    await loadData();
    showOperationStatus('success', 'Leave approved successfully');
  } catch (err: any) {
    showOperationStatus('error', err.message || 'Failed to approve leave');
  } finally {
    setOperationLoading(false);
  }
};
            ...
            <tr>
              ...
              >
                <div className="flex items-center space-x-1">
                  <span>Type</span>
                  <ChevronDown className={`w-4 h-4 ${sortBy === 'type' ? 'text-primary-600' : 'text-gray-400'}`} />
                </div>
              </th>
        ...
        >
          <div className="flex flex-col items-center space-y-3 mb-3">
            ...
            <div className="text-center w-full">
              <div className="font-semibold text-gray-900 text-xs truncate">{formatLeaveType(leave.type)}</div>
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-3">
              <div>
                ...
                <p className="text-xs text-gray-500">Create, view and manage employee leave requests</p>
              </div>
            </div>
            <div className="flex items-center space-x-3">
              <motion.button
          ...
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="fixed inset-0 bg-black/50 flex items-center justify-center z-50 p-4">
            <div className="bg-white rounded-lg p-6 w-full max-w-md shadow-xl">
              ...
              <div className="flex items-center justify-end space-x-3">
                ...
                <motion.button whileHover={{ scale: 1.05 }} onClick={() => handleDeleteLeave(deleteConfirm)} className="px-4 py-2 text-xs bg-red-600 text-white rounded hover:bg-red-700">
                  Delete
                </motion.button>
              </div>
            </div>
      ...
      <div className="bg-white rounded-lg p-6 w-full max-w-md shadow-xl">
        <div className="flex items-center space-x-3 mb-4">
          ...
          <div>
            ...
            <p className="text-xs text-gray-500">You may add an optional approval note</p>
          </div>
        </div>
        ...
        <div className="mb-4">
          <p className="text-xs text-gray-700 mb-3">
            ...
            <span className="font-semibold">
              {approveConfirm.employee?.first_name || approveConfirm.employee?.last_name
                ? `${approveConfirm.employee?.first_name || ''} ${approveConfirm.employee?.last_name || ''}`.trim()
                : 'Employee'}
            </span>
            ?
          </p>
          ...
          <textarea
            ...
            placeholder="Optional: Add approval note or message (e.g., 'Enjoy your vacation!')"
          ...
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="fixed inset-0 bg-black/50 flex items-center justify-center z-50 p-4">
            <div className="bg-white rounded-lg p-6 w-full max-w-md shadow-xl">
              ...
              <div className="flex items-center justify-end space-x-3">
                ...
                <motion.button whileHover={{ scale: 1.05 }} onClick={handleRejectLeave} disabled={!rejectReason.trim()} className="px-4 py-2 text-xs bg-red-600 text-white rounded hover:bg-red-700 disabled:opacity-50">
                  Reject
                </motion.button>
              </div>
            </div>
          ...
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="fixed inset-0 bg-black/50 flex items-center justify-center z-50 p-4">
            <div className="bg-white rounded-lg p-6 w-full max-w-lg shadow-xl overflow-y-auto max-h-screen">
              ...
              <div className="space-y-4 mb-4">
                ...
                <div>
                  <label className="block text-xs font-medium text-gray-700 mb-1">Attachments (Images/PDF)</label>
              ...
              <div className="flex items-center justify-end space-x-3">
                ...
                <motion.button whileHover={{ scale: 1.05 }} onClick={handleCreateOrUpdateLeave} disabled={operationLoading} className="px-4 py-2 text-xs bg-primary-600 text-white rounded hover:bg-primary-700 disabled:opacity-50">
