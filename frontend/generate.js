...
const sourceImage = './public/logo/aby_dash_white.png';
...
// iOS Splash Screen Specifications (unchanged dimensions)
const splashSpecs = [
    // iPhone
...
async function generateAllAssets() {
    console.log('Generating comprehensive PWA assets – now with WHITE backgrounds instead of primary color...\n');
    ...
    if (!fs.existsSync(sourceImage)) {
        ...
        console.log('Please place your logo at ./public/logo/aby_dash.png (1024×1024 recommended)');
        return;
    }

    try {
        // === PWA icons (transparent or white padding) ===
        ...
        // === Android Maskable Icons – now WHITE background ===
        console.log('\nGenerating Android Maskable Icons (white background)...');
        for (const icon of iconSpecs.maskable) {
            ...
            await sharp({
                create: {
                    ...
                    height: icon.size,
                    channels: 4,
                    background: { r: 255, g: 255, b: 255, alpha: 1 } // ← WHITE instead of primary blue
                }
            })
            .composite([{
        ...
        // === Windows Tiles – now WHITE background ===
        console.log('\nGenerating Windows Tiles (white background)...');
        ...
        // === iOS Splash Screens – now pure WHITE background ===
        console.log('\nGenerating iOS Splash Screens (white background)...');
        for (const splash of splashSpecs) {
            ...
            await sharp({
                create: {
                    ...
                    channels: 4,
                    background: { r: 255, g: 255, b: 255, alpha: 1 } // ← WHITE instead of light blue
                }
            })
            .composite([{
                input: await sharp(sourceImage)
        ...
        console.log('\nAll assets generated successfully with WHITE backgrounds!');
        console.log('\nGenerated:');
